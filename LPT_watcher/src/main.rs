...
async fn main() {
    ...
    println!("Round manager contract interface instanciated");
    ...
    let reward_stream = (&mut reward_subscription).for_each(|log| {
        ...
        if transaction_detector.has_valid_transaction_been_made(init.web3(), log) {
